...
if arguments.contains("--explain") {
    ...
    for f in history.currentForecasts(now: now) where !f.isReset {
        ...
        let input = EstimationInput(window: f.window, now: now, past: history.pastSeries(tool: f.tool, minutes: f.window.minutes, scope: f.window.scope, before: now))
        ...
        if let pattern, let e = pattern.explain(input) {
            ...
            let peak = max(e.usual.max() ?? 0, 1e-9)
